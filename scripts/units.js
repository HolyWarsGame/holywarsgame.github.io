...
var Unit = function(name, htmlNumRef, htmlNextCostRef, htmlBuyBtn, goldCost, ironCost, silverCost, faithCost, soulCost, costMult,description, costAdj, hasReqUnit, reqUnit){
	this.name = name;
...
Unit.prototype.buyOne = function(){
	...
	if(this.hasReqUnit == false || (this.hasReqUnit == true && this.reqUnit.returnNumber() > 0)){
		if(gold >= this.curCost && iron >= this.ironCost && silver >= this.silverCost && faith >= this.faithCost && souls >= this.soulCost ){    //checks that the player can afford the Unit
...
var Page = new Unit("Page",'personPage','PageCost','btnBuyPage',500,100,0,0,0,1.1, pageDesc, 0, false, "none");
...
var Squire = new Unit("Squire",'squires','SquireCost','btnBuySquire',1200,250,0,0,0,1.1, squireDesc, 0, true, Page);	
...
var Knight = new Unit("Knight",'knights','KnightCost','btnBuyKnight',3000,350,0,0,0,1.25, knightDesc, 0, true, Squire);	
...
var Paladin = new Unit("Paladin",'paladins','PaladinCost','btnBuyPaladin',10000,500,100,50,0,1.1, paladinDesc, 0, true, Knight);		
...
var Acolyte = new Unit("Acolyte",'acolytes','AcolyteCost','btnbuyAcolyte',500,0,0,0,0,1.1, acolyteDesc, 0, false, "none");	
